...
function App() {
  ...
  const startAction = () => {
    ...
    window.setTimeout(() => {
      ...
      const outputExt = activeToolData.output;
      const baseName = newest.name.replace(/\.[^.]+$/, "");
      const generatedName =
        activeTool === "merge-pdf"
          ? `merged-document${outputExt}`
          : activeTool === "split-pdf"
          ? `${baseName}-split${outputExt}`
          : `${baseName}-converted${outputExt}`;
        ...
        <section className="grid gap-8 lg:grid-cols-2 lg:items-center lg:pt-6">
          <div>
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-slate-300 sm:text-lg">
              ...
              PDF to Word, PDF merging, and more. The visual flow is ready for the
              real conversion engine later.
            </p>
            ...
            >
              <div className="flex flex-col items-center justify-center rounded-[1.4rem] border border-white/8 bg-white/5 px-6 py-10 text-center">
                ...
                <p className="mt-2 max-w-md text-sm leading-6 text-slate-300">
                  Drag and drop documents, or choose them from your device. This UI
                  is ready for backend conversion logic.
                </p>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
                This design is ready. The backend can later handle DOCX → PDF, PDF → DOCX,
                and PDF merge/split using server-side file processing.
              </p>
            </div>
            <button className="inline-flex items-center gap-2 rounded-full bg-cyan-300 px-5 py-2.5 text-sm font-semibold text-slate-950">
              Ready to build backend <ChevronRight className="h-4 w-4" />
            </button>
          </div>
